...
export class PolandPhoneNumber implements PhoneNumber<"PL"> {
  iso = "PL" as const;
  metadata = new PhoneNumberMetadata("48", "Poland");
  format(phoneNumber: string) {
    const significantNumberLength = 9;
    const significantNumber = refinePhoneNumber(
      phoneNumber,
    ...
    switch (true) {
      ...
      case significantNumber.length <= 3:
        ...
        break;
      default:
        formattedSignificantNumber = `${significantNumber.slice(0, 3)} ${significantNumber.slice(3, significantNumberLength)}`;
        break;
    }
