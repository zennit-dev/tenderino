...
import { DAYS_OF_THE_WEEK, MONTHS_OF_THE_YEAR, type Month } from "../_utils";
...
export const CalendarBody = ({
  ...
  const lastWeeksSundayOfMonth = endOfWeek(lastDayOfMonth, { weekStartsOn: 1 });
      ...
      <div className={"flex gap-1"}>
        {DAYS_OF_THE_WEEK.map((day) => (
          <div
            key={day}
            className={
              "flex aspect-square w-[calc((100%/7)-theme(spacing.1))] items-center justify-center text-accent-foreground text-xs"
            }
          >
            {day.slice(0, 2)}
          </div>
